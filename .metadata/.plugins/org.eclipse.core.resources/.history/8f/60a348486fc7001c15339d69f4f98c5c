...
public class Login {
    ...
    public void autenticarUsuario() {
        for(int i = 0;  i < Leitor.getClientes().size(); i++) {
            if(this.cpf.equals(Leitor.getClientes().get(i).getCpf())
                && this.senha.equals(Leitor.getClientes().get(i).getSenha())) {
                cliente = Leitor.getClientes().get(i);
                autenticidade = true;
            } 
        }
    }
